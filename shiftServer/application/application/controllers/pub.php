...
class Pub extends CI_Controller {
    ...
    public function achivement($id){
        ...
        echo json_encode($this->achievment_pub_model->getAchivementPage($id));
    }
    
    public function achivementSocial($id){
        $this->load->model('public/achievment_pub_model');
        $data = $this->achievment_pub_model->getAchivmenetForSocial($id);
        if($data['success']){
            $this->load->helper('canelio_server_helper');
            $data['data']['angUrl'] = getAngularUrl().'#/achivement/'.$id;
